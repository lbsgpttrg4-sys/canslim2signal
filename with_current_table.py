...
# Compute score
# -----------------------------
def compute_indicators_and_score(prices: pd.Series, cfg: StrategyConfig) -> pd.Series:
    trend_weight = 1.0
    rsi_weight = 0.8
...
def app():
    ...
    if tickers_input:
        ...
        if results:
            # Main display DataFrame
            df = pd.DataFrame(results, columns=["Date", "Ticker", "Live Price", "Regime", "Analyst",
                                                "Score", "SMA_Long", "EMA_Short", "RSI", "ATR", "ADX"])

            # --- New table for calculations ---
            current_table = df.copy()
            current_table["Current Price"] = current_table["Live Price"]
            current_table.set_index("Ticker", inplace=True)
            ...
            st.write("Current Table for Calculation:")
